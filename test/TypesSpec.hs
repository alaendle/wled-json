  ...
  describe "Types" $ do
    ...
    describe "StateComplete" $ do
      ...
      it "append/diff" $
        property prop
      it "what" $ do
        let c :: StateComplete = State {stateOn = False, stateBri = 1, stateTransition = 1, statePs = 1, statePl = 0, stateNl = Nightlight {nightlightOn = True, nightlightDur = 0, nightlightMode = -1, nightlightTbri = -1, nightlightRem = -1}, stateLor = 0, stateMainseg = -1, stateSeg = [Segment {segmentId = 0, segmentStart = 1, segmentStop = -1, segmentLen = -1, segmentGrp = 0, segmentSpc = 1, segmentOf = 0, segmentOn = True, segmentFrz = True, segmentBri = -1, segmentCct = 1, segmentSet = 0, segmentCol = [], segmentFx = 0, segmentSx = 1, segmentIx = -1, segmentPal = 0, segmentC1 = 0, segmentC2 = -1, segmentC3 = 0, segmentSel = False, segmentRev = False, segmentMi = False, segmentO1 = False, segmentO2 = True, segmentO3 = False, segmentSi = -1, segmentM12 = -1}]}
        let p1 :: StatePatch = State {stateOn = Just False, stateBri = Just 1, stateTransition = Just 0, statePs = Just 0, statePl = Just (-1), stateNl = Just (Nightlight {nightlightOn = Just False, nightlightDur = Just 0, nightlightMode = Just 1, nightlightTbri = Just 1, nightlightRem = Just 0}), stateLor = Just 0, stateMainseg = Just 1, stateSeg = Just [Segment {segmentId = Just 1, segmentStart = Nothing, segmentStop = Just 1, segmentLen = Just 0, segmentGrp = Just (-1), segmentSpc = Just 1, segmentOf = Nothing, segmentOn = Just True, segmentFrz = Nothing, segmentBri = Just 1, segmentCct = Just (-1), segmentSet = Just (-1), segmentCol = Just [[-1]], segmentFx = Just 0, segmentSx = Just 1, segmentIx = Just 0, segmentPal = Nothing, segmentC1 = Just (-1), segmentC2 = Nothing, segmentC3 = Nothing, segmentSel = Just True, segmentRev = Just False, segmentMi = Nothing, segmentO1 = Just False, segmentO2 = Nothing, segmentO3 = Just False, segmentSi = Just (-1), segmentM12 = Nothing}]}
        let p2 :: StatePatch = State {stateOn = Just True, stateBri = Nothing, stateTransition = Nothing, statePs = Nothing, statePl = Just (-1), stateNl = Just (Nightlight {nightlightOn = Just True, nightlightDur = Nothing, nightlightMode = Just 1, nightlightTbri = Just (-1), nightlightRem = Just (-1)}), stateLor = Just 1, stateMainseg = Just 1, stateSeg = Just []}
        --print $ p2 <> p1
        --print $ append c p1
        --print $ append (append c p1) p2
        --print $ append c (p2 <> p1)
        append c (p2 <> p1) `shouldBe` append (append c p1) p2
